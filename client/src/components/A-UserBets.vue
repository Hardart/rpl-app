<template>
  <div class="grid childs-w-100 childs-w-50@s childs-w-33@m childs-w-25@l" v-if="betStore.myBets.length > 0">
    <div v-for="bet in betStore.myBets">
      <div class="bet-card">
        <ATeamCard :event="event(bet.event_id)" :show-bet-score="true" />
        <div class="bet-card__status">
          <div class="rotate-180">
            {{ checkBetStatus(bet, score(bet.event_id), event(bet.event_id).status) }}
          </div>
        </div>
      </div>
    </div>
