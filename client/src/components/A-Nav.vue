  ...
  <nav class="nav">
    <ul class="nav-list">
      <template v-for="menuItem in menu">
        <router-link custom v-slot="{ navigate, isActive }" :to="menuItem.link" v-if="!menuItem.auth || playerStore.isLogin">
          <li class="nav-list__item btn btn-icon btn-success" :class="{ active: isActive }" @click="navigate" role="link" v-html="menuItem.icon"></li>
        </router-link>
      </template>
