...
class EventController {
  ...
  async loadPastEvents(req, res, next) {
    try {
      ...
      const { roundGroup, eventsCount } = await eventService.finished(limit)
      if (eventsCount) res.setHeader('X-Total-Count', eventsCount)
      return res.status(200).json(roundGroup)
    } catch (error) {
      next(error)
    }
  }
